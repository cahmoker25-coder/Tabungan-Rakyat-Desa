...
function switchProfileTab(tabName) {
  ...
  const passTab = document.getElementById("profile-tab-pass");
  ...
  if (tabName === 'info') {
    ...
    btnInfo.style.background = "#10b981";
    btnPass.style.background = "#334155";
  } else {
    infoTab.classList.add("hidden");
    passTab.classList.remove("hidden");
    btnPass.style.background = "#10b981";
    btnInfo.style.background = "#334155";
  }
}
...
function bindUserEvents() {
  ...
  if(btnSaveProf) {
    btnSaveProf.addEventListener("click", function() {
      const newUsernameInput = document.getElementById("edit-username").value.trim().toLowerCase().replace("@", "");
      if (!newUsernameInput) return alert("Username tidak boleh kosong!");

      if (newUsernameInput === currentUser.username) {
        return alert("Tidak ada perubahan username.");
      }

      db.ref("users/" + newUsernameInput).once("value", (snap) => {
        if (snap.exists()) {
        ...
        db.ref("users/" + currentUser.username).once("value", (oldSnap) => {
          ...
          userData.username = newUsernameInput;

          db.ref("users/" + newUsernameInput).set(userData, (err) => {
            if (!err) {
              db.ref("users/" + currentUser.username).remove();
              currentUser = userData;
              localStorage.setItem("loggedUser", JSON.stringify(userData));
              alert("🎉 Username berhasil diubah! Silakan lanjutkan sesi Anda.");
              closeModal('modal-profil');
              location.reload();
            } else {
              alert("Gagal mengubah username: " + err.message);
            }
          });
        });
      });
  ...
  if(btnSavePass) {
    btnSavePass.addEventListener("click", function() {
      ...
      const confNewPass = document.getElementById("conf-new-pass-input").value;

      if (!oldPass || !newPass || !confNewPass) return alert("Lengkapi semua kolom sandi!");
      if (oldPass !== currentUser.password) return alert("Sandi lama Anda salah!");
      
      const valRes = validateComplexPassword(newPass);
      if (!valRes.isValid) return alert("Sandi baru harus minimal 8 karakter, ada huruf kapital & kecil, angka, dan simbol (!?;:*&$,@)!");
      if (newPass !== confNewPass) return alert("Konfirmasi sandi baru tidak cocok!");

      db.ref("users/" + currentUser.username).update({ password: newPass }, (err) => {
        if (!err) {
          ...
          document.getElementById("conf-new-pass-input").value = "";
          closeModal('modal-profil');
        } else {
          alert("Gagal memperbarui sandi: " + err.message);
        }
  ...
  if(btnPertemanan) btnPertemanan.addEventListener("click", () => { loadFriendRequests(); loadFriendsList(); populateAvailableUsersForFriend(); openModal('modal-pertemanan'); });
...
function submitKirimSaldo() {
  ...
  db.ref("users/" + targetUser).once("value", (snap) => {
    ...
    db.ref("transactions").push({
      ...
      if (!err) {
        alert(`Permintaan transfer Rp ${amount.toLocaleString('id-ID')} ke @${targetUser} dikirim ke Admin untuk verifikasi.`);
...
function accMoneyRequest(trxId) {
  db.ref("transactions/" + trxId).once("value", (snap) => {
    ...
    db.ref("users/" + currentUser.username).once("value", (meSnap) => {
      let myBal = Number(meSnap.val().balance) || 0;
      if (myBal < amount) {
        return alert("Saldo Anda tidak mencukupi untuk menyetujui permintaan ini!");
      }

      db.ref("users/" + currentUser.username).update({ balance: myBal - amount });
      ...
      let resiInfo = "TRX-REQ-" + Math.floor(100000 + Math.random() * 900000);
      db.ref("transactions/" + trxId).update({ 
        status: "Disetujui", 
        type: `Minta Saldo ke @${currentUser.username} (Disetujui)`,
        receiptCode: resiInfo 
      });
...
function sendFriendRequest() {
  ...
  if (!target) return alert("Pilih warga terlebih dahulu dari daftar!");
...
function listenUserChat() {
  ...
  db.ref("chats/" + currentUser.username).on("value", (snap) => {
    ...
    if (chats) {
      ...
      box.innerHTML = `<div class="empty-state">Belum ada obrolan dengan Admin. Ketik pesan di bawah untuk berkonsultasi!</div>`;
    }
  });
}
...
// ==========================================
// 8. LOGIKA DASHBOARD ADMIN (ACC PRESISI, PENGURANGAN SALDO KIRIM/TARIK & RESI)
// ==========================================
...
function renderAdminAllData() {
  ...
  Object.keys(globalUsers).forEach(username => {
    ...
    if (u && u.role !== 'admin') {
      ...
      if (tbMod) tbMod.innerHTML += `
        <tr>
          ...
          <td>
            ...
            <button class="btn-action btn-del" onclick="hapusUser('${username}')">Hapus Akun</button>
          </td>
        </tr>`;
      ...
      if (tbDataWarga) tbDataWarga.innerHTML += `
        <tr>
          <td>${u.fullname||'-'}</td>
          <td>@${username}</td>
          <td>${u.email||'-'}</td>
          ...
          <td>${u.birthDate||'-'}</td>
          <td>${u.motherName||'-'}</td>
          <td><span style="font-family:monospace; color:#38bdf8; font-weight:bold;">${u.password || '-'}</span></td>
        </tr>`;
    }
  });
...
function accTrx(trxKey) {
  ...
  if (tType === "nabung") {
    ...
    db.ref("users/" + trx.username).update(updates, (err) => {
      if (!err) {
        let recCode = "TRX-REC-" + Math.floor(100000 + Math.random() * 900000);
        db.ref("transactions/" + trxKey).update({ status: "Disetujui", receiptCode: recCode });
        alert("🎉 Transaksi Nabung disetujui & Saldo bertambah!");
      }
    });
  } 
  // 2. PINJAMAN (SALDO BERTAMBAH)
  else if (tType.includes("pinjaman")) {
    bal += amt;
    db.ref("users/" + trx.username).update({ balance: bal, status: "active" }, (err) => {
      if (!err) {
        let recCode = "TRX-REC-" + Math.floor(100000 + Math.random() * 900000);
        db.ref("transactions/" + trxKey).update({ status: "Disetujui", receiptCode: recCode });
  ...
  // 3. TARIK SALDO (SALDO BERKURANG)
    ...
    if (bal < amt) {
      return alert("Gagal ACC: Saldo warga tidak mencukupi untuk penarikan ini!");
    }
    bal -= amt;
    
    db.ref("users/" + trx.username).update({ balance: bal }, (err) => {
      if (!err) {
        let recCode = "TRX-WD-" + Math.floor(100000 + Math.random() * 900000);
        db.ref("transactions/" + trxKey).update({ status: "Disetujui", receiptCode: recCode });
...
-------------------------------------
💬 PEMBERITAHUAN:
Penarikan saldo Anda telah berhasil diproses oleh Bendahara. Silakan cek akun rekening bank atau dompet digital Anda secara berkala. Terima kasih!
        `.trim();
        ...
        if (targetUser) {
          db.ref("users/" + targetUser).once("value", (tSnap) => {
            ...
            db.ref("transactions").push({
              ...
              time: new Date().toLocaleString('id-ID'),
              rawDate: new Date().toISOString().split('T')[0],
              receiptCode: "TRX-RCV-" + Math.floor(100000 + Math.random() * 900000)
            });
          });
        }

        let recCode = "TRX-TRF-" + Math.floor(100000 + Math.random() * 900000);
        db.ref("transactions/" + trxKey).update({ status: "Disetujui", receiptCode: recCode });
  ...
  // 5. LAINNYA
    ...
    let recCode = "TRX-REC-" + Math.floor(100000 + Math.random() * 900000);
    db.ref("transactions/" + trxKey).update({ status: "Disetujui", receiptCode: recCode }, (err) => {
      if (!err) alert("🎉 Transaksi berhasil di-ACC!");
    });
  }
}
