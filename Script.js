...
function bindUserEvents() {
  ...
  // 4 Tombol Sekunder (Ganti prompt alert dengan modal form rapi)
...
function payArisanAuto() {
  ...
  // Cek apakah hari ini sudah bayar arisan
  db.ref("transactions").orderByChild("username").equalTo(currentUser.username).once("value", (snap) => {
    ...
    // Potong saldo langsung dan catat sebagai Disetujui (atau Pending sesuai alur, namun langsung potong saldo agar logis)
...
function accTrx(trxKey) {
  ...
  if (trx.type === "Nabung") {
    ...
    // Jika transaksi setoran awal memenuhi syarat minimal Rp 50.000, aktifkan akun warga otomatis
...
function kocokArisan() {
  ...
  if (confirm(`🎉 PEMENANG ARISAN:\n${winName} (@${winner.username})\n\nTransfer Rp ${totalKasArisan.toLocaleString('id-ID')} ke akunnya?`)) {
    ...
    // Hapus transaksi iuran arisan sebelumnya atau catat pengeluaran kas arisan agar kas arisan berkurang secara akurat
