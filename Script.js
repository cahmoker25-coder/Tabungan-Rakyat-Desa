...
// ==========================================
// 2. LOGIKA UTAMA SETELAH DOM READY
// ==========================================
document.addEventListener("DOMContentLoaded", function () {
  ...
  // A. Inisialisasi Halaman Auth (index.html)
  ...
  if (boxLogin) {
    ...
    initUsernameAutoSuggest();
    initLoginUserHandler();
    initRegisterUserHandler();
  }

  // B. Inisialisasi Login Admin (loginadmin.html)
  ...
  // C. Inisialisasi Dashboard User (user-dashboard.html)
  const valBalance = document.getElementById("val-balance");
  if (valBalance) {
    initUserDashboard();
  }

  // D. Inisialisasi Dashboard Admin (admin.html)
...
// ==========================================
// 3. LOGIKA VALIDASI SANDI AUTOMATIS
// ==========================================
function validateComplexPassword(pass) {
  ...
  return { 
    ...
    hasNum, 
    hasSymbol, 
    isValid: minLength && hasCaps && hasNum && hasSymbol 
  };
}
...
function initRealtimePasswordCheck(passInputId, confInputId, matchStatusId, rulesPrefix) {
  ...
  if (passInput) {
    passInput.addEventListener("input", function () {
      ...
      if (confInput && confInput.value.length > 0) {
        checkMatch();
      }
    });
  }
...
// ==========================================
// 4. NAVIGASI & AUTHENTICATION USER (INDEX)
// ==========================================
  ...
  if (formLogin && db) {
    formLogin.addEventListener("submit", function (e) {
      ...
      db.ref("users").once("value", (snapshot) => {
        ...
        db.ref("users/" + foundUser.username).update({ lastLogin: nowStr });
...
function initRegisterUserHandler() {
  ...
  if (formDaftar && db) {
    formDaftar.addEventListener("submit", function (e) {
      ...
      if (!passVal.isValid) {
        return alert("Peringatan Sandi Baru: Harus minimal 8 karakter dengan kombinasi Huruf Kapital, Huruf Kecil, Angka, dan Simbol (!?;:*&$,@)!");
      }
      if (password !== confirmPassword) {
        return alert("Peringatan: Konfirmasi sandi baru tidak cocok dengan sandi di atas!");
      }
        ...
        if (snap.exists()) {
          ...
          db.ref("users/" + username).set(newUserObject, (err) => {
            if (err) {
              ...
              alert("🎉 Pendaftaran Berhasil! Anda mendapatkan Bonus Saldo Rp 100.000. Silakan login.");
              formDaftar.reset();
              switchForm('login');
            }
          });
...
// ==========================================
// 5. LUPA PASSWORD 5 STEP HANDLER
// ==========================================
...
function resetStep3() {
  ...
  if (resetUserData.birthDate === inputDob) {
    ...
    alert("Tanggal lahir tidak cocok dengan data terdaftar!");
  }
}
...
function resetStep4() {
  ...
  if (userPob === pob && resetUserData.pin === inputPin) {
    ...
    alert("Kota kelahiran atau PIN Transaksi SALAH!");
  }
}
...
function resetStep5() {
  ...
  if (!valRes.isValid) {
    return alert("Peringatan Sandi Baru: Minimal 8 karakter dengan kombinasi Huruf Kapital, Huruf Kecil, Angka, dan Simbol (!?;:*&$,@)!");
  }
  if (newPass !== confPass) {
    return alert("Peringatan: Konfirmasi sandi baru tidak cocok dengan sandi baru di atas!");
  }
  ...
  if (db) {
    db.ref("users/" + u).once("value", (snap) => {
      if (snap.exists() && snap.val().password === p && snap.val().role === "admin") {
        sessionStorage.setItem("loggedUser", JSON.stringify(snap.val()));
        alert("🔓 Otentikasi Berhasil!");
        window.location.href = "admin.html";
      } else {
        alert("Akses Ditolak: ID atau Password Admin Salah!");
      }
    });
  } else {
...
function bindUserEvents() {
  ...
  const btnFilterType = document.getElementById("filter-type");
  const btnFilterStart = document.getElementById("filter-start-date");
  const btnFilterEnd = document.getElementById("filter-end-date");
  if(btnFilterType) btnFilterType.addEventListener("change", applyHistoryFilter);
  if(btnFilterStart) btnFilterStart.addEventListener("change", applyHistoryFilter);
  if(btnFilterEnd) btnFilterEnd.addEventListener("change", applyHistoryFilter);
...
// FUNGSI KIRIM SALDO ANTAR WARGA REALTIME
...
function applyHistoryFilter() {
  ...
  if (filterType !== "ALL") filtered = filtered.filter(t => t.type.includes(filterType));
  if (startDate) filtered = filtered.filter(t => (t.rawDate || "") >= startDate);
  if (endDate) filtered = filtered.filter(t => (t.rawDate || "") <= endDate);
...
function renderAdminAllData() {
  ...
  Object.keys(globalTrxs).reverse().forEach(key => {
    ...
    if (trx.status === 'Disetujui' && trx.type === 'Iuran Arisan') totalKasArisan += trx.amount;
      ...
      if (tbStatus) {
        tbStatus.innerHTML += `
          <tr>
            ...
            <td>@${username}</td>
            <td>${u.lastLogin || '-'}</td>
            <td><span class="badge ${u.status==='banned'?'badge-danger':'badge-success'}">${u.status==='banned'?'Banned/Offline':'Aktif'}</span></td>
          </tr>`;
      }

      if (tbDataWarga) {
        const vNama = u.fullname || u.nama || username;
        const vEmail = u.email || '-';
        const vPhone = u.phone || u.nohp || u.no_hp || '-';
        const vKota = u.birthPlace || u.pob || u.kotaLahir || '-';
        const vBank = u.bank || u.eWallet || u.namaBank || '-';
        const vNamaRek = u.accountName || u.namaRekening || u.namaPemilik || '-';
        const vNoRek = u.accountNumber || u.noRekening || u.norek || '-';
        const vTglLahir = u.birthDate || u.dob || u.tglLahir || '-';
        const vIbu = u.motherName || u.mother || u.ibuKandung || '-';
        const vPin = u.pin || '------';
        const vPass = u.password || u.pass || '******';
        ...
        tbDataWarga.innerHTML += `
          <tr>
            ...
            <td>${vPhone}</td>
            <td>${vKota}</td>
            <td><b style="color:#10b981;">${vBank}</b></td>
            <td><b>${vNamaRek}</b></td>
            <td><span style="font-family:monospace; color:#fff;">${vNoRek}</span></td>
            <td>${vTglLahir}</td>
            <td>${vIbu}</td>
            <td><span style="font-family:monospace; color:#f59e0b;">${vPin}</span></td>
            <td><span style="font-family:monospace; color:#ff3b5c;">${vPass}</span></td>
          </tr>`;
      }
    }
  });
...
// LOGIKA VERIFIKASI ACC DENGAN AUTO-TRANSFER SALDO & RIWAYAT OTOMATIS
function accTrx(trxKey) {
  ...
  // C. KIRIM / TRANSFER SALDO ANTAR WARGA
    ...
    if (!recipientUser) {
      return alert("Gagal ACC: Username penerima tidak ditemukan di database!");
    }
  ...
  // Update Status Transaksi Pengirim Menjadi Disetujui
  db.ref("transactions/" + trxKey).update({ status: "Disetujui" }, (err) => {
    if (!err) {
      alert("🎉 Transaksi Berhasil Di-ACC! Saldo & Riwayat Otomatis Terupdate.");
    }
  });
}

function rejTrx(trxKey) {
  db.ref("transactions/" + trxKey).update({ status: "Ditolak" });
}

function deleteTrx(trxKey) {
  if (confirm("Hapus transaksi dari riwayat cloud?")) {
    db.ref("transactions/" + trxKey).remove();
  }
}
...
function hapusTeguran(username) {
  if (confirm(`Hapus teguran untuk @${username}?`)) {
    db.ref("users/" + username + "/warning").remove();
  }
}
...
function kocokArisan() {
  ...
  Object.keys(globalTrxs).forEach(k => {
    let t = globalTrxs[k];
    if (t.type === 'Iuran Arisan' && t.status === 'Disetujui') totalKasArisan += t.amount;
  });
  ...
  if (confirm(`🎉 PEMENANG ARISAN:\n${winName} (@${winner.username})\n\nTransfer Rp ${totalKasArisan.toLocaleString('id-ID')} ke akunnya?`)) {
    ...
    const timeStr = new Date().toLocaleDateString('id-ID') + " " + new Date().toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'});
    db.ref("transactions").push({ username: winner.username, type: "Pemenang Arisan", amount: totalKasArisan, status: "Disetujui", time: timeStr });
    db.ref("arisan_winner").set(`${winName} — Rp ${totalKasArisan.toLocaleString('id-ID')}`);
    alert(`BERHASIL! Saldo dikirim ke @${winner.username}! Banner diterbitkan.`);
  }
}

function hapusPemenang() {
  if (confirm("Bersihkan banner pemenang arisan?")) {
    db.ref("arisan_winner").set("");
  }
}
