  ...
  db.ref("transactions").orderByChild("username").equalTo(currentUser.username).once("value", (snap) => {
    ...
    if(snap.exists()) {
      snap.forEach(child => {
        ...
        if(tType.includes("iuran arisan") && t.rawDate === todayStr) {
          if(t.status === "Pending") {
            canSubmit = false;
...
// ==========================================
// FIX UTAMA: RENDER ADMIN DATA & KAS ARISAN BERSIH
// ==========================================
function renderAdminAllData() {
  ...
  Object.keys(globalTrxs).reverse().forEach(key => {
    ...
    // Hitung mutlak jika disetujui (mendukung format iuran arisan, baik huruf besar/kecil)
      ...
      if(tbVerif) {
        tbVerif.innerHTML += `
          <tr>
            ...
            <td><b style="color:${tType.includes('iuran') ? '#f59e0b' : '#38bdf8'}">${trx.type}</b></td>
...
// ==========================================
// FIX MUTLAK ACC ADMIN (POTONG SALDO & JAMIN TIPE TERCATAT 'Iuran Arisan')
// ==========================================
function accTrx(trxKey) {
  ...
  // Update status dan pastikan format type tertulis 'Iuran Arisan' persis agar terbaca sempurna
  db.ref("transactions/" + trxKey).update({ 
    status: "Disetujui",
    type: (tType.includes("iuran") || tType.includes("arisan")) ? "Iuran Arisan" : trx.type
  }, (err) => {
    if (!err) {
      alert("🎉 Transaksi Berhasil Di-ACC! Saldo Warga Terpotong & Total Kas Arisan Otomatis Bertambah.");
    } else {
...
function kocokArisan() {
  ...
  Object.keys(globalTrxs).forEach(k => {
    ...
    if (t && t.status === 'Disetujui') {
      let tType = (t.type || "").toLowerCase().trim();
      if (tType.includes('iuran') && tType.includes('arisan')) totalIuranMasuk += Number(t.amount) || 0;
      if (tType.includes('pencairan') || tType.includes('pemenang') || Number(t.amount) < 0) totalPencairanKeluar += Math.abs(Number(t.amount) || 0);
    }
  });
