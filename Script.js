...
// Mengambil data P2P murni secara terpisah dari node `p2p_requests` dengan penyortiran rapi
function fetchP2PRequests() {
  db.ref("p2p_requests").on("value", (snap) => {
    const p2ps = snap.val();
    
    // Hapus data P2P lama dari array global agar tidak menumpuk dobel
    globalTransactions = globalTransactions.filter(item => !item.isP2P);

    if (p2ps) {
      Object.keys(p2ps).forEach(key => {
        const t = p2ps[key];
    ...
    // Urutkan mutlak dari transaksi terbaru ke terlama berdasarkan tanggal/waktu
    globalTransactions.sort((a, b) => new Date(b.rawDate || 0) - new Date(a.rawDate || 0));
...
function bindUserEvents() {
  ...
  // Hubungkan tombol tab profil & ubah sandi
  const btnTabInfo = document.getElementById("tab-btn-info");
  const btnTabPass = document.getElementById("tab-btn-pass");
  const btnSavePass = document.getElementById("btn-save-password");

  if (btnTabInfo) btnTabInfo.addEventListener("click", () => switchProfileTab('info'));
  if (btnTabPass) btnTabPass.addEventListener("click", () => switchProfileTab('pass'));
  if (btnSavePass) btnSavePass.addEventListener("click", submitChangePassword);
...
// ==========================================
// MODUL SWITCH TAB PROFIL & UBAH SANDI
// ==========================================
function switchProfileTab(tabName) {
  const tabInfo = document.getElementById("profile-tab-info");
  const tabPass = document.getElementById("profile-tab-pass");
  ...
  if (tabName === 'info') {
    ...
    if (btnInfo) { btnInfo.style.background = "#10b981"; btnInfo.style.opacity = "1"; }
    if (btnPass) { btnPass.style.background = "#334155"; btnPass.style.opacity = "0.7"; }
  } else {
    tabInfo.classList.add("hidden");
    tabPass.classList.remove("hidden");
    if (btnPass) { btnPass.style.background = "#10b981"; btnPass.style.opacity = "1"; }
    if (btnInfo) { btnInfo.style.background = "#334155"; btnInfo.style.opacity = "0.7"; }
  }
}
...
function submitChangePassword() {
  ...
  const confPass = document.getElementById("conf-new-pass-input").value;

  if (!oldPass || !newPass || !confPass) return alert("Lengkapi semua form ubah sandi!");
  if (oldPass !== currentUser.password) return alert("Sandi lama salah!");
  ...
  db.ref("users/" + currentUser.username).update({ password: newPass }, (err) => {
    if (!err) {
      alert("🎉 Sandi berhasil diubah! Silakan gunakan sandi baru untuk login selanjutnya.");
