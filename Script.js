...
function renderAdminAllData() {
  ...
  Object.keys(globalTrxs).reverse().forEach(key => {
    ...
    // KALKULASI BERSIH: Akumulasi masuk dan keluar
    if (trx.status === 'Disetujui') {
      if (tType.includes('iuran') && tType.includes('arisan')) {
        totalIuranArisanMasuk += amt;
      } 
      // TANGKAP SEMUA PENGELUARAN PENCAIRAN ARISAN / ANGKA MINUS SECARA MUTLAK
      else if (tType.includes('pencairan') || tType.includes('pemenang') || amt < 0) {
        totalPencairanArisanKeluar += Math.abs(amt);
      }
    }
      ...
      if(tbVerif) {
        tbVerif.innerHTML += `
          <tr>
            ...
            <td><b>Rp ${(amt || 0).toLocaleString('id-ID')}</b></td>
    ...
    if (tbIuran && tType && (tType.includes("iuran") || tType.includes("sumbangan") || tType.includes("zakat"))) {
      tbIuran.innerHTML += `
        <tr>
          ...
          <td><b>@${trx.username || '-'}</b></td>
          <td>${trx.type}</td>
          <td>Rp ${(amt || 0).toLocaleString('id-ID')}</td>
...
function kocokArisan() {
  ...
  Object.keys(globalTrxs).forEach(k => {
    ...
    if (t && t.status === 'Disetujui') {
      ...
      if (tType.includes('iuran') && tType.includes('arisan')) totalIuranMasuk += amt;
      if (tType.includes('pencairan') || tType.includes('pemenang') || amt < 0) totalPencairanKeluar += Math.abs(amt);
    }
  });
  ...
  if (confirm(`🎉 PEMENANG ARISAN:\n${winName} (@${winner.username})\n\nTransfer Rp ${nominalPemenang.toLocaleString('id-ID')} secara otomatis ke akunnya?`)) {
    ...
    // Catat penambahan saldo ke pemenang
    db.ref("transactions").push({ 
    ...
    // CATAT PENGELUARAN NEGATIF AGAR KAS ARISAN LANGSUNG LUDES JADI 0
    db.ref("transactions").push({ 
      username: "SYSTEM", 
