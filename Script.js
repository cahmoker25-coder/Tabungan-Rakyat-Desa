...
function submitMintaSaldo() {
  ...
  db.ref("users/" + targetUser).once("value", (snap) => {
    ...
    // Diberi penanda isP2P: true agar admin mutlak mengabaikannya
    db.ref("transactions").push({
      username: currentUser.username, // Warga yang meminta
      targetUser: targetUser,         // Warga yang dimintai saldo
...
function acceptP2PRequest(trxKey) {
  ...
  db.ref("users/" + targetUser).once("value", (bSnap) => {
    ...
    db.ref("users/" + requester).once("value", (aSnap) => {
      ...
      // Saldo warga B berkurang otomatis, saldo warga A bertambah otomatis
      let updates = {};
...
function renderAdminAllData() {
  ...
  Object.keys(globalTrxs).reverse().forEach(key => {
    ...
    // PENTING: Jika ada penanda isP2P atau teks mengandung permintaan saldo antarwarga, MUTLAK DIABAIKAN OLEH ADMIN
    if (trx.isP2P || (trx.type && trx.type.includes("Permintaan Saldo"))) {
      return; 
    }
