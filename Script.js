...
function renderAdminAllData() {
  ...
  Object.keys(globalTrxs).reverse().forEach(key => {
    ...
    // PERBAIKAN MUTLAK: Pisahkan secara mutlak total iuran masuk dan pencairan keluar
        if (trx.status === 'Disetujui') {
      if (trx.type === 'Iuran Arisan') {
        totalIuranArisanMasuk += trx.amount;
      } else if (trx.type === 'Pencairan Kas Arisan' || trx.type === 'Pemenang Arisan') {
        totalPencairanArisanKeluar += Math.abs(trx.amount);
      }
    }
  ...
  // HITUNG AKHIR KAS ARISAN BERSIH
...
function kocokArisan() {
  ...
  let totalPencairanKeluar = 0;
    ...
    if (t.status === 'Disetujui') {
      if (t.type === 'Iuran Arisan') totalIuranMasuk += t.amount;
      if (t.type === 'Pencairan Kas Arisan' || t.type === 'Pemenang Arisan') totalPencairanKeluar += Math.abs(t.amount);
    }
  });
  ...
  if (confirm(`🎉 PEMENANG ARISAN:\n${winName} (@${winner.username})\n\nTransfer Rp ${nominalPemenang.toLocaleString('id-ID')} ke akunnya?`)) {
    ...
    // CATAT PENGELUARAN KAS ARISAN BERSIH SEBESAR NOMINAL KAS AKTIF (MENJADI 0)
    db.ref("transactions").push({ 
      username: "SYSTEM", 
