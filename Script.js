    ...
    formDaftar.addEventListener("submit", function (e) {
      ...
      db.ref("users/" + username).once("value", (snap) => {
        if (snap.exists()) {
          ...
          // Status awal 'pending_verification' agar fitur transaksi terkunci sampai setoran awal Rp 50.000 di-ACC
          const newUserObject = {
          ...
          db.ref("users/" + username).set(newUserObject, (err) => {
            if (err) alert("Gagal Mendaftar: " + err.message);
              ...
              alert("🎉 Pendaftaran Berhasil! Anda mendapat bonus Rp 100.000 (terkunci). Lakukan setoran awal min. Rp 50.000 agar akun di-ACC Bendahara.");
...
function bindUserEvents() {
  ...
  // ATURAN 2: Validasi akun aktif untuk tombol transaksi utama
  if(btnKirim) btnKirim.addEventListener("click", () => {
    if(currentUser.status !== "active") return alert("⚠️ Akses Transaksi Terkunci: Lakukan setoran awal minimal Rp 50.000 melalui menu 'Isi Saldo' dan tunggu konfirmasi ACC dari Bendahara untuk membuka fitur ini.");
  ...
  if(btnTarik) btnTarik.addEventListener("click", () => {
    if(currentUser.status !== "active") return alert("⚠️ Akses Transaksi Terkunci: Lakukan setoran awal minimal Rp 50.000 dan tunggu ACC Bendahara untuk menarik saldo.");
    openModal('modal-tarik');
  });
  
  // Pembayaran iuran, sumbangan, zakat juga wajib aktif
...
// LOGIKA PEMBAYARAN ARISAN: Cek max 1x sehari, kirim sebagai Pending ke admin
...
function renderAdminAllData() {
  ...
  Object.keys(globalTrxs).reverse().forEach(key => {
    ...
    // Total Kas Arisan HANYA BERTAMBAH jika statusnya 'Disetujui' (di-ACC Admin)
    if (trx.status === 'Disetujui') {
      if (trx.type === 'Iuran Arisan') {
        totalIuranArisanMasuk += Number(trx.amount) || 0;
      } else if (trx.type === 'Pencairan Kas Arisan' || trx.type === 'Pemenang Arisan' || Number(trx.amount) < 0) {
  ...
  // AKUMULASI KAS ARISAN BERSIH (MASUK - KELUAR)
...
// ==========================================
// ATURAN 1 & 3: POTONG SALDO WARGA & MASUKKAN KE KAS ARISAN HANYA SAAT DI-ACC ADMIN
// ==========================================
function accTrx(trxKey) {
  ...
  if (trx.type === "Nabung") {
    ...
    // Jika setoran awal memenuhi syarat minimal Rp 50.000, aktifkan akun warga secara resmi
    if (trx.amount >= 50000 && senderUser.status !== "active") {
      updates.status = "active";
    }
    db.ref("users/" + trx.username).update(updates);
  } else if (trx.type === "Iuran Arisan") {
    if (senderBal < trx.amount) return alert("Gagal ACC: Saldo warga tidak mencukupi untuk membayar iuran arisan!");
    senderBal -= trx.amount;
  ...
  db.ref("transactions/" + trxKey).update({ status: "Disetujui" }, (err) => {
    if (!err) alert("🎉 Transaksi Berhasil Di-ACC! Saldo Warga Terpotong & Total Kas Arisan Bertambah.");
  });
}
...
// ==========================================
// ATURAN 4: KAS ARISAN JADI 0 SAAT PEMENANG DITRANSFER OTOMATIS
// ==========================================
function kocokArisan() {
  ...
  Object.keys(globalTrxs).forEach(k => {
    ...
    if (t.status === 'Disetujui') {
      if (t.type === 'Iuran Arisan') totalIuranMasuk += Number(t.amount) || 0;
      if (t.type === 'Pencairan Kas Arisan' || t.type === 'Pemenang Arisan' || Number(t.amount) < 0) totalPencairanKeluar += Math.abs(Number(t.amount) || 0);
    }
  });
  ...
  if (confirm(`🎉 PEMENANG ARISAN:\n${winName} (@${winner.username})\n\nTransfer Rp ${nominalPemenang.toLocaleString('id-ID')} secara otomatis ke akunnya?`)) {
    let newBal = (winner.balance || 0) + nominalPemenang;
    ...
    // CATAT PENGELUARAN KAS ARISAN BERSIH SEbesar NOMINAL KAS AKTIF (MEMBUAT SISA KAS RESET MENJADI 0)
    db.ref("transactions").push({ 
      username: "SYSTEM", 
...
function triggerWithdrawalToast() {
  ...
  toastEl.classList.add("hidden"); // placeholder safe toggle
}
