...
function initUserDashboard() {
  ...
  db.ref("transactions").on("value", (snap) => {
    ...
    fetchP2PRequests(); // Gabungkan dengan data P2P
  });
...
// Listener khusus untuk mengambil data permintaan saldo antarwarga (P2P) secara terpisah dari admin
    ...
    if (p2ps) {
      Object.keys(p2ps).reverse().forEach(key => {
        ...
        if (t.requester === currentUser.username || t.targetUser === currentUser.username) {
          // Cek apakah sudah ada di globalTransactions, jika belum tambahkan
          ...
          if (!exists) {
            ...
            // Update status jika berubah di database
...
function submitMintaSaldo() {
  ...
  db.ref("users/" + targetUser).once("value", (snap) => {
    ...
    // Disimpan mutlak ke node terpisah `p2p_requests` (TIDAK AKAN MASUK KE ADMIN)
...
function renderUserTransactions() {
  ...
  globalTransactions.forEach((t) => {
    ...
    // Tombol ACC/Tolak mutlak muncul di akun warga yang dimintai saldo (targetUser) saat status Pending
    let p2pActionHtml = '';
