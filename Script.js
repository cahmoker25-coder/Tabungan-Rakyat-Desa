  ...
  db.ref("transactions").on("value", (snap) => {
    ...
    if (trxs) {
      Object.keys(trxs).reverse().forEach(key => {
        const t = trxs[key];
        // Ditambahkan t.targetUser === currentUser.username agar riwayat permintaan saldo muncul di akun warga yang dimintai
...
// ==========================================
// FITUR P2P: MINTA SALDO ANTARWARGA (TANPA ADMIN)
// ==========================================
...
function submitMintaSaldo() {
  ...
  db.ref("users/" + targetUser).once("value", (snap) => {
    ...
    db.ref("transactions").push({
      ...
      targetUser: targetUser,         // Warga yang dimintai saldo (harus ACC/Tolak sendiri)
...
function acceptP2PRequest(trxKey) {
  ...
  db.ref("users/" + targetUser).once("value", (bSnap) => {
    ...
    db.ref("users/" + requester).once("value", (aSnap) => {
      ...
      // Saldo warga B berkurang, saldo warga A bertambah secara otomatis
      let updates = {};
...
function renderUserTransactions() {
  ...
  globalTransactions.forEach((t) => {
    ...
    // Jika transaksi adalah Permintaan Saldo P2P dan currentUser adalah targetUser yang harus ACC/Tolak saat status Pending
    let p2pActionHtml = '';
    if (t.status === 'Pending' && t.targetUser === currentUser.username && t.type && t.type.includes('Permintaan Saldo')) {
      p2pActionHtml = `
...
function renderAdminAllData() {
  ...
  Object.keys(globalTrxs).reverse().forEach(key => {
    ...
    // Transaksi P2P "Permintaan Saldo" ditangani warga secara mandiri, sehingga tidak masuk ke daftar pending admin
