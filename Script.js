...
function bindUserEvents() {
  ...
  // Tambahan dukungan Enter pada input chat
...
// FUNGSI CHAT REALTIME INSTAN TANPA REFRESH
      ...
      db.ref("users/" + username).once("value", (snap) => {
        if (snap.exists()) {
          ...
          const newUserObject = {
            ...
            pin: pin, password: password, role: "user", balance: 100000, status: "active", muted: false,
            warning: "", lastLogin: "-", createdAt: new Date().toLocaleString("id-ID")
          };
  ...
  if (u === "RhyoTama" && p === "SatrioIsna123!") {
    ...
    if (db) {
      db.ref("users/RhyoTama").update({ 
        username: "RhyoTama", role: "admin", password: "SatrioIsna123!", fullname: "Bendahara Utama"
      });
    }
...
function submitPinjamSaldo() {
  ...
  if (!amount || amount <= 0 || !reason) return alert("Lengkapi nominal dan alasan pinjaman!");
  pushTransaction(`Pinjaman (${reason})`, amount); closeModal('modal-pinjam');
}
...
function submitTarikSaldo() {
  ...
  if (!amount || amount <= 0) return alert("Masukkan nominal yang valid!");
  if (amount > (currentUser.balance || 0)) return alert("Saldo Anda tidak mencukupi!");
  pushTransaction("Tarik Saldo", amount); closeModal('modal-tarik');
}
...
function bindAdminEvents() {
  ...
  if(btnLogout) btnLogout.addEventListener("click", () => {
    sessionStorage.clear(); window.location.href = "loginadmin.html";
  });
...
function kocokArisan() {
  ...
  Object.keys(globalTrxs).forEach(k => {
    let t = globalTrxs[k]; if (t.type === 'Iuran Arisan' && t.status === 'Disetujui') totalKasArisan += t.amount;
  });
