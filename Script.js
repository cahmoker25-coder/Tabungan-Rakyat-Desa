...
function initUserDashboard() {
  ...
  db.ref("users/" + currentUser.username).on("value", (snap) => {
    if (snap.exists()) {
      ...
      document.getElementById("log-status-akun").innerText = currentUser.status === "active" ? "Aktif (Terverifikasi)" : "Menunggu Setoran Awal (Min. Rp 50.000)";[span_2](start_span)[span_2](end_span)[span_3](start_span)[span_3](end_span)
...
function accTrx(trxKey) {
  ...
  // Ambil data terbaru langsung dari database Firebase untuk akurasi saldo
  db.ref("users/" + username).once("value", (userSnap) => {
    ...
    // 3. JIKA TARIK SALDO (SALDO BERKURANG MUTLAK)
      ...
      bal -= amt; // Pengurangan saldo mutlak
...
Penarikan saldo Anda telah berhasil diproses dan saldo akun Anda telah dipotong. Silakan cek akun rekening bank atau dompet digital Anda secara berkala. Terima kasih!
    ...
    // 4. JIKA TRANSFER / KIRIM SALDO (SALDO PENGIRIM BERKURANG MUTLAK)
