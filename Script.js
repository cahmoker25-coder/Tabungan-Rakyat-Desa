...
function renderAdminAllData() {
  ...
  Object.keys(globalTrxs).reverse().forEach(key => {
    ...
    if (trx.status === 'Disetujui') {
      if (tType.includes('iuran arisan')) {
        totalIuranArisanMasuk += Number(trx.amount) || 0;
      } else if (tType.includes('pencairan') || tType.includes('pemenang arisan') || Number(trx.amount) < 0) {
...
function kocokArisan() {
  ...
  Object.keys(globalTrxs).forEach(k => {
    ...
    if (t && t.status === 'Disetujui') {
      ...
      if (tType.includes('iuran arisan')) totalIuranMasuk += Number(t.amount) || 0;
      if (tType.includes('pencairan') || tType.includes('pemenang arisan') || Number(t.amount) < 0) totalPencairanKeluar += Math.abs(Number(t.amount) || 0);
    }
  });
