...
function initUserDashboard() {
  ...
  populateAvailableUsersForMinta(); // Populate dropdown target warga untuk minta saldo
}
...
function bindUserEvents() {
  ...
  // Fitur transaksi dikunci menggunakan checkAccess()
  ...
  // Auto-generate & handle input chat teman secara dinamis langsung dari Script.js
...
// ==========================================
// FITUR BARU: MINTA SALDO ANTARWARGA (P2P)
// ==========================================
...
function submitMintaSaldo() {
  ...
  db.ref("users/" + targetUser).once("value", (snap) => {
    ...
    // Disimpan dengan targetUser sebagai penanggung jawab ACC/Tolak (bukan admin)
    db.ref("transactions").push({
      username: currentUser.username, // Warga yang meminta
      targetUser: targetUser,         // Warga yang dimintai saldo (harus ACC/Tolak)
...
function acceptP2PRequest(trxKey) {
  ...
  db.ref("users/" + targetUser).once("value", (bSnap) => {
    ...
    let bBal = Number(bData.balance) || 0;
    ...
    db.ref("users/" + requester).once("value", (aSnap) => {
      ...
      let aBal = Number(aData.balance) || 0;

      // Kurangi saldo warga B (targetUser) & Tambah saldo warga A (requester)
      let updates = {};
      updates[`users/${targetUser}/balance`] = bBal - amt;
      updates[`users/${requester}/balance`] = aBal + amt;
      updates[`transactions/${trxKey}/status`] = "Disetujui";
      updates[`transactions/${trxKey}/receiptCode`] = "P2P-" + Math.floor(100000 + Math.random() * 900000);
...
function rejectP2PRequest(trxKey) {
  db.ref("transactions/" + trxKey).update({ status: "Ditolak" }, (err) => {
    if (!err) {
      alert("Permintaan saldo telah ditolak.");
    }
  });
}
// ==========================================
...
function renderUserTransactions() {
  ...
  globalTransactions.forEach((t) => {
    ...
    // Jika transaksi adalah P2P Minta Saldo dan currentUser adalah targetUser yang harus ACC/Tolak saat status Pending
    let p2pActionHtml = '';
    if (t.status === 'Pending' && t.targetUser === currentUser.username && t.type.includes('Permintaan Saldo')) {
      p2pActionHtml = `
...
// ==========================================
// 9. LOGIKA DASHBOARD ADMIN (ACC PRESISI, PENGURANGAN SALDO & PEMBUATAN RESI)
// ==========================================
...
function renderAdminAllData() {
  ...
  Object.keys(globalTrxs).reverse().forEach(key => {
    ...
    // Transaksi P2P "Permintaan Saldo" ditangani warga secara mandiri, jadi TIDAK MASUK ke daftar pending admin
    if (trx.type && trx.type.includes("Permintaan Saldo")) {
      return; 
    }
...
function accTrx(trxKey) {
  ...
  db.ref("users/" + username).once("value", (userSnap) => {
    ...
    // 3. TARIK SALDO (SALDO BERKURANG MUTLAK)
    else if (tType.includes("tarik saldo")) {
      if (bal < amt) {
        return alert(`Gagal ACC: Saldo warga (${bal.toLocaleString('id-ID')}) tidak mencukupi untuk penarikan sebesar Rp ${amt.toLocaleString('id-ID')}!`);
      }
      bal -= amt;
...
-------------------------------------
💬 PEMBERITAHUAN:
Penarikan saldo Anda telah berhasil diproses dan saldo akun Anda telah dipotong. Silakan cek akun rekening bank atau dompet digital Anda secara berkala. Terima kasih!
          `.trim();
          alert(resiAlert);
        }
      });
    } 
    // 4. TRANSFER / KIRIM SALDO (SALDO PENGIRIM BERKURANG MUTLAK)
    else if (tType.includes("transfer ke")) {
      if (bal < amt) {
        return alert(`Gagal ACC: Saldo pengirim (${bal.toLocaleString('id-ID')}) tidak mencukupi!`);
      }
      bal -= amt;
