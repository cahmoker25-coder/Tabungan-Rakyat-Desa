...
function bindUserEvents() {
  ...
  // Tombol Simpan Perubahan Username di Profil
  ...
  // Tombol Simpan Perubahan Sandi di Profil
...
// ==========================================
// 8. LOGIKA DASHBOARD ADMIN (ACC PINJAMAN & RESI TARIK SALDO)
// ==========================================
...
function renderAdminAllData() {
  ...
  Object.keys(globalUsers).forEach(username => {
    ...
    if (u && u.role !== 'admin') {
      ...
      // Database Warga (Sandi Terlihat Jelas)
...
function accTrx(trxKey) {
  ...
  if (tType === "nabung") {
    ...
    db.ref("transactions/" + trxKey).update({ status: "Disetujui" }, () => alert("Transaksi di-ACC & Saldo bertambah!"));
  } 
  else if (tType.includes("pinjaman")) {
    let pinjamAmt = Number(trx.amount) || 0;
    bal += pinjamAmt;
    db.ref("users/" + trx.username).update({ balance: bal, status: "active" });
    db.ref("transactions/" + trxKey).update({ status: "Disetujui" }, () => alert(`🎉 Pinjaman Rp ${pinjamAmt.toLocaleString('id-ID')} disetujui! Saldo akun peminjam kini bertambah menjadi Rp ${bal.toLocaleString('id-ID')}.`));
  } 
  else if (tType.includes("tarik saldo")) {
    let tarikAmt = Number(trx.amount) || 0;
    if (bal < tarikAmt) {
    ...
    let kodeTrx = "TRX-WD-" + Math.floor(100000 + Math.random() * 900000);
...
• Nominal Tarik  : Rp ${tarikAmt.toLocaleString('id-ID')}
...
Penarikan saldo Anda telah berhasil diproses oleh Bendahara. Silakan cek akun rekening bank atau dompet digital Anda secara berkala. Terima kasih!
    ...
    alert(resiAlert);
    db.ref("transactions/" + trxKey).update({ status: "Disetujui" });
  } else {
