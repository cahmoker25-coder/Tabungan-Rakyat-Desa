...
function bindUserEvents() {
  ...
  if(btnKirim) btnKirim.addEventListener("click", () => {
    if(currentUser.status !== "active") return alert("⚠️ Akses Transaksi Terkunci: Lakukan setoran awal minimal Rp 50.000 di menu 'Isi Saldo' dan tunggu konfirmasi ACC dari Bendahara.");
    openModal('modal-kirim');
  });
  
  if(btnArisan) btnArisan.addEventListener("click", () => {
    if(currentUser.status !== "active") return alert("⚠️ Akses Transaksi Terkunci: Selesaikan setoran awal minimal Rp 50.000 terlebih dahulu.");
    payArisanAuto();
  });
  ...
  if(btnTarik) btnTarik.addEventListener("click", () => {
    if(currentUser.status !== "active") return alert("⚠️ Akses Transaksi Terkunci: Lakukan setoran awal minimal Rp 50.000 dan tunggu ACC Bendahara.");
    openModal('modal-tarik');
  });
  
  if(btnIuran) btnIuran.addEventListener("click", () => {
    if(currentUser.status !== "active") return alert("⚠️ Akses Transaksi Terkunci: Akun belum terverifikasi setoran awal.");
    openModalSec('Iuran Warga');
  });
  if(btnSumbangan) btnSumbangan.addEventListener("click", () => {
    if(currentUser.status !== "active") return alert("⚠️ Akses Transaksi Terkunci: Akun belum terverifikasi setoran awal.");
    openModalSec('Sumbangan Kas');
  });
  if(btnZakat) btnZakat.addEventListener("click", () => {
    if(currentUser.status !== "active") return alert("⚠️ Akses Transaksi Terkunci: Akun belum terverifikasi setoran awal.");
    openModalSec('Zakat & Infaq');
  });
...
function payArisanAuto() {
  ...
  db.ref("transactions").orderByChild("username").equalTo(currentUser.username).once("value", (snap) => {
    ...
    if(snap.exists()) {
      snap.forEach(child => {
        let t = child.val();
        if(t.type && t.type.toLowerCase().includes("iuran arisan") && t.rawDate === todayStr) {
          if(t.status === "Pending") {
            canSubmit = false;
            alert("⚠️ Anda sudah mengajukan pembayaran iuran arisan hari ini dan masih menunggu verifikasi Admin.");
          } else if(t.status === "Disetujui") {
            canSubmit = false;
            alert("⚠️ Anda sudah membayar iuran arisan hari ini dan telah disetujui Admin. Tidak boleh membayar lagi.");
          }
        }
      });
    }
    ...
    db.ref("transactions").push({
      username: currentUser.username, type: "Iuran Arisan", amount: 25000, status: "Pending",
      time: new Date().toLocaleDateString('id-ID') + " " + new Date().toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'}),
      rawDate: todayStr
    }, (err) => {
      if(!err) {
        alert("🎉 Pengajuan Iuran Arisan Rp 25.000 berhasil dikirim ke Admin untuk diverifikasi.");
        switchNav('beranda', document.querySelectorAll('.nav-btn')[0]);
      } else {
        alert("Gagal mengajukan iuran arisan: " + err.message);
      }
    });
  });
}
...
// ==========================================
// FIX KAS ARISAN BERSIH DENGAN PENGECEKAN FLEKSIBEL (`INCLUDES`)
// ==========================================
function renderAdminAllData() {
  let totalKasTabungan = 0;
  let totalIuranArisanMasuk = 0;
  let totalPencairanArisanKeluar = 0;
  let countPending = 0;
  ...
  Object.keys(globalTrxs).reverse().forEach(key => {
    ...
    // Konversi string tipe agar aman dari perbedaan huruf besar/kecil/spasi
    let tType = (trx.type || "").toLowerCase().trim();
    
        if (trx.status === 'Disetujui') {
      // Cek apakah tipenya mengandung kata 'iuran' dan 'arisan' (tidak peduli huruf besar/kecil)
      let tType = (trx.type || "").toLowerCase();
      if (tType.includes('iuran') && tType.includes('arisan')) {
        totalIuranArisanMasuk += Number(trx.amount) || 0;
      } else if (tType.includes('pencairan') || tType.includes('pemenang') || Number(trx.amount) < 0) {
        totalPencairanArisanKeluar += Math.abs(Number(trx.amount) || 0);
      }
    }
      ...
      if(tbVerif) {
        tbVerif.innerHTML += `
          <tr>
            ...
            <td><b style="color:${tType.includes('iuran arisan') ? '#f59e0b' : '#38bdf8'}">${trx.type}</b></td>
            <td><b>Rp ${(Number(trx.amount) || 0).toLocaleString('id-ID')}</b></td>
    ...
    if (tbIuran && tType && (tType.includes("iuran") || tType.includes("sumbangan") || tType.includes("zakat"))) {
      tbIuran.innerHTML += `
        <tr>
          <td>${trx.time || '-'}</td>
          <td><b>@${trx.username || '-'}</b></td>
      ...
      if (tbSaldo) {
        tbSaldo.innerHTML += `
          <tr>
            ...
            <td style="color:#10b981; font-weight:bold;">Rp ${(Number(u.balance)||0).toLocaleString('id-ID')}</td>
            <td>
  ...
  // TOTAL KAS ARISAN AKHIR BERSIH
  let totalKasArisan = totalIuranArisanMasuk - totalPencairanArisanKeluar;
  if (totalKasArisan < 0) totalKasArisan = 0;
...
// ==========================================
// FIX MUTLAK ACC ADMIN (POTONG SALDO WARGA & PASTIKAN TIPE TERCATAT 'Iuran Arisan')
// ==========================================
function accTrx(trxKey) {
  ...
  if (tType === "nabung") {
  ...
  } else if (tType.includes("iuran arisan")) {
    ...
    if (senderBal < (Number(trx.amount) || 0)) return alert("Gagal ACC: Saldo warga pengirim tidak mencukupi!");
    senderBal -= Number(trx.amount) || 0;
    db.ref("users/" + trx.username).update({ balance: senderBal });
  ...
  // Update status transaksi menjadi 'Disetujui' secara mutlak
  db.ref("transactions/" + trxKey).update({ 
    status: "Disetujui",
    type: tType.includes("iuran arisan") ? "Iuran Arisan" : trx.type 
  }, (err) => {
    if (!err) {
      alert("🎉 Transaksi Berhasil Di-ACC! Saldo Warga Terpotong & Total Kas Arisan Bertambah.");
    } else {
...
// ==========================================
// KAS ARISAN JADI 0 SAAT PEMENANG DITRANSFER OTOMATIS
// ==========================================
function kocokArisan() {
  ...
  let totalPencairanKeluar = 0;

    Object.keys(globalTrxs).forEach(k => {
    let t = globalTrxs[k]; 
    if (t && t.status === 'Disetujui') {
      let tType = (t.type || "").toLowerCase();
      if (tType.includes('iuran') && tType.includes('arisan')) {
        totalIuranMasuk += Number(t.amount) || 0;
      }
      if (tType.includes('pencairan') || tType.includes('pemenang') || Number(t.amount) < 0) {
        totalPencairanKeluar += Math.abs(Number(t.amount) || 0);
      }
    }
  });


  let totalKasArisanAktif = totalIuranMasuk - totalPencairanKeluar;
  if (totalKasArisanAktif <= 0) return alert("Kas Arisan kosong atau sudah habis dicairkan!");
  ...
  let nominalPemenang = totalKasArisanAktif;

  if (confirm(`🎉 PEMENANG ARISAN:\n${winName} (@${winner.username})\n\nTransfer Rp ${nominalPemenang.toLocaleString('id-ID')} secara otomatis ke akunnya?`)) {
    let newBal = (Number(winner.balance) || 0) + nominalPemenang;
    ...
    db.ref("transactions").push({ 
      ...
      type: "Pemenang Arisan", 
      amount: nominalPemenang, 
      status: "Disetujui", 
      time: timeStr, 
      rawDate: rawDateStr 
    });
    ...
    db.ref("arisan_winner").set(`${winName} — Rp ${nominalPemenang.toLocaleString('id-ID')}`);
    alert(`BERHASIL! Saldo sebesar Rp ${nominalPemenang.toLocaleString('id-ID')} otomatis ditransfer ke @${winner.username}. Total Kas Arisan kini reset menjadi Rp 0!`);
  }
}
