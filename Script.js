...
function initUserDashboard() {
  ...
  // Listener transaksi umum dari bendahara/admin
  db.ref("transactions").on("value", (snap) => {
    ...
    fetchP2PRequests(); // Gabungkan dengan data P2P murni antarwarga
  });
...
// Listener khusus untuk mengambil data P2P murni dari node `p2p_requests`
...
// DIPERBAIKI: Murni kirim ke `p2p_requests` (TIDAK AKAN MASUK KE ADMIN SAMA SEKALI)
function submitMintaSaldo() {
  ...
  db.ref("users/" + targetUser).once("value", (snap) => {
    ...
    db.ref("p2p_requests").push({
      requester: currentUser.username, // Warga yang minta
...
// DIPERBAIKI: Eksekusi mutlak antarwarga (saldo warga B berkurang, warga A bertambah)
...
// DIPERBAIKI: Jika ditolak, saldo kedua belah pihak TIDAK BERUBAH SAMA SEKALI
...
function renderUserTransactions() {
  ...
  globalTransactions.forEach((t) => {
    ...
    // DIPERBAIKI: Tombol ACC/Tolak mutlak muncul di akun warga yang dimintai saldo (targetUser) saat status Pending
    let p2pActionHtml = '';
...
// ==========================================
// 9. LOGIKA DASHBOARD ADMIN (DIJAMIN BERSIH DARI P2P)
// ==========================================
...
function renderAdminAllData() {
  ...
  Object.keys(globalTrxs).reverse().forEach(key => {
    ...
    // PROTEKSI MUTLAK: Filter garis keras agar admin tidak bisa melihat/memproses permintaan saldo antarwarga
    if (trx.type && (trx.type.includes("Permintaan Saldo") || trx.type.includes("Minta Saldo"))) {
      return; 
    }
