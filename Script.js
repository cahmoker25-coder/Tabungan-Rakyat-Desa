...
// LOGIKA PEMBAYARAN ARISAN: Wajib verifikasi admin (Pending), cek 1x sehari via rawDate
function payArisanAuto() {
  ...
  db.ref("transactions").orderByChild("username").equalTo(currentUser.username).once("value", (snap) => {
    ...
    if(snap.exists()) {
      snap.forEach(child => {
        let t = child.val();
        // Cek apakah hari ini sudah ada pengajuan Iuran Arisan (baik Pending, Disetujui, maupun Ditolak)
        if(t.type === "Iuran Arisan" && t.rawDate === todayStr) {
          ...
          // Jika status 'Ditolak', warga diperbolehkan mengajukan ulang di hari yang sama
        }
      });
    }
    ...
    // Catat sebagai Pending (saldo belum dipotong otomatis sebelum di-ACC admin)
    db.ref("transactions").push({
...
function renderAdminAllData() {
  ...
  Object.keys(globalTrxs).reverse().forEach(key => {
    ...
    // PERBAIKAN: Total Kas Arisan hanya bertambah jika Iuran Arisan berstatus 'Disetujui' (di-ACC Admin)
  ...
  // HITUNG AKHIR KAS ARISAN BERSIH (MASUK - KELUAR)
...
// LOGIKA ADMIN ACC: Potong saldo warga saat Iuran Arisan di-ACC
function accTrx(trxKey) {
  ...
  db.ref("transactions/" + trxKey).update({ status: "Disetujui" }, (err) => {
    if (!err) alert("🎉 Transaksi Berhasil Di-ACC! Saldo & Total Kas Arisan Otomatis Terupdate.");
  });
}
...
function kocokArisan() {
  ...
  if (confirm(`🎉 PEMENANG ARISAN:\n${winName} (@${winner.username})\n\nTransfer Rp ${nominalPemenang.toLocaleString('id-ID')} ke akunnya?`)) {
    ...
    // CATAT PENGELUARAN KAS ARISAN BERSIH SEBESAR NOMINAL KAS AKTIF (MEMBUAT SISA KAS MENJADI 0)
    db.ref("transactions").push({ 
      username: "SYSTEM", 
    ...
    alert(`BERHASIL! Saldo kas arisan sebesar Rp ${nominalPemenang.toLocaleString('id-ID')} telah dipotong (Sisa Kas: Rp 0) dan dikirim ke @${winner.username}!`);
  }
}
