...
// ==========================================
// 5. LUPA PASSWORD (DISEDERHANAKAN TANPA PIN)
// ==========================================
let resetUserData = null;
let resetStepLevel = 1;
...
// ==========================================
// 7. DASHBOARD USER & PENGAMANAN BAN
// ==========================================
let currentSecType = "";
...
function initUserDashboard() {
  ...
  db.ref("users/" + currentUser.username).on("value", (snap) => {
    if (snap.exists()) {
      ...
      // Jika pengguna di-ban, langsung tolak akses dan logout otomatis
...
function bindUserEvents() {
  ...
  // PENGAMANAN TRANSAKSI: Blokir mutlak jika banned atau belum active (setoran awal 50rb)
  const checkUserAccess = () => {
    ...
    if (currentUser.status !== "active") {
      alert("⚠️ Akses Transaksi Terkunci: Lakukan setoran awal minimal Rp 50.000 melalui menu 'Isi Saldo' dan tunggu konfirmasi ACC dari Bendahara.");
      return false;
    }
    return true;
  };
...
// ==========================================
// 8. LOGIKA DASHBOARD ADMIN & FITUR HAPUS SEMUA RIWAYAT
// ==========================================
...
// Fungsi Hapus Semua Riwayat Transaksi di Cloud Database
function clearAllTransactions() {
  if (confirm("⚠️ PERINGATAN: Apakah Anda yakin ingin MENGHAPUS SEMUA riwayat transaksi secara permanen dari database? Tindakan ini tidak dapat dibatalkan!")) {
    if (db) {
      db.ref("transactions").remove().then(() => {
        alert("🗑️ Seluruh riwayat transaksi berhasil dihapus.");
      }).catch((error) => {
...
function renderAdminAllData() {
  ...
  Object.keys(globalUsers).forEach(username => {
    ...
    if (u && u.role !== 'admin') {
      ...
      // PANEL DATABASE WARGA (PIN 6 DIGIT DIHAPUS/TIDAK DITAMPILKAN)
      if (tbDataWarga) {
