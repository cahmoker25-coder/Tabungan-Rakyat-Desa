...
function renderAdminAllData() {
  ...
  Object.keys(globalTrxs).reverse().forEach(key => {
    ...
    // PERBAIKAN: Kalkulasi Kas Arisan akurat (Masuk & Keluar)
...
function kocokArisan() {
  ...
  let nominalPemenang = totalKasArisan;
  ...
  if (confirm(`🎉 PEMENANG ARISAN:\n${winName} (@${winner.username})\n\nTransfer Rp ${nominalPemenang.toLocaleString('id-ID')} ke akunnya?`)) {
    ...
    // PERBAIKAN: Catat pengeluaran kas arisan bernilai negatif persis sebesar nominal yang dicairkan
    db.ref("transactions").push({ 
      username: "SYSTEM", 
    ...
    alert(`BERHASIL! Saldo kas arisan telah dipotong sebesar Rp ${nominalPemenang.toLocaleString('id-ID')} dan dikirim ke @${winner.username}!`);
  }
}
